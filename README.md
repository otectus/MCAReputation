...
| **+ MCA: Crime** | Can register as a core-incident authority for villager assault and killing through `McaReputationApi.registerCoreIncidentAuthority` while `enableCrimeIntegration` is on; which kinds it actually claims depends on the Crime version installed and the `coreAuthorityUndeclaredKinds` setting |
...
(the two kinds every such companion could have detected before per-kind declaration existed).
...
/mcareputation debug community|witnesses|integrations|authorities|standing [<player>] [<community>]
