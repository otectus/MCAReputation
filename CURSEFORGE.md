...
### A reputation with the village, not a villager
...
### Nine ways a village can see you
...
titles, and the scrollable list of what they remember. It is drawn from texture sprites in
...
| **Required** | [MCA Reborn](https://www.curseforge.com/minecraft/mc-mods/minecraft-comes-alive-reborn) NeoForge `7.7.x` |
| **Optional** | MCA: Quests (1.21.1) — quests, projects, and situations become named public deeds |
| **Optional** | MCA: Conversations (1.21.1) — villagers gossip about your actual deeds and treat you accordingly |
| **Optional** | MCA: Crime (1.21.1) — safe to install alongside; the two mods agree on who detects villager harm |

> This is an **add-on** — MCA Reborn must be installed for it to do anything.

**Reflection-bound MCA integration.** This mod resolves MCA by name at runtime, so it runs on the full
`7.7.x` range. A missing MCA member logs one startup error and degrades the feature instead of crashing
the server. At startup the log states whether integration came up — that line is the first thing to
...
With **MCA: Quests** (1.1.0+) installed, Reputation becomes the single source of truth for village
...
*Requires this mod's API version 2.*

## Optional: MCA: Conversations integration

With **MCA: Conversations** (2.0.0+) installed, the ledger comes alive. Villagers who know a deed
...
or inspect the legacy migration. A `debug` branch answers support questions directly: `debug standing`
prints everything the standing pipeline believes about one player, and `debug authorities` shows who
currently claims what. Every mutation is audit-logged with who did it and why.
...
`registerCoreIncidentAuthority` so the two never double-count. Targeting API version 2 on this
platform; bridges built against version 1 must re-target and recompile.

---

## Status & license

**0.3.0** — first public release on NeoForge 1.21.1, actively developed; feedback and bug reports welcome.
